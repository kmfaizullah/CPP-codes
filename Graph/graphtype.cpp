...
GraphType<VertexType>::BreadthFirstSearch(VertexType startVertex, VertexType endVertex)
{
    ...
    do
    {
        ...
        if (vertex == endVertex)
        {
            cout << vertex << " ";
            found = true;
            //cout<<"Length of smallest path is : 1"<<endl;
        }
        else
        {
            if (!IsMarked(vertex))
...
void GraphType<VertexType>::BreadthFirstSearch(VertexType startVertex, VertexType endVertex)
{
    QueType<VertexType> queue;
    QueType<VertexType> vertexQ;
    StackType<VertexType> st;
    int lth=0;

    cout << endl;
    VertexType prevNode [numVertices];
    for(int i = 0 ; i < numVertices; i++)
    {
        prevNode[i] = vertices[i];
